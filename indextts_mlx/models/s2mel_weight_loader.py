...
def create_dit_weight_mapper() -> WeightMapper:
    ...
    # Transformer layers
    mapper.add_mapping('estimator.transformer.layers.{i}.attention.wqkv.weight', 'transformer.layers.{i}.attention.wqkv.weight')
    mapper.add_mapping('estimator.transformer.layers.{i}.attention.wo.weight', 'transformer.layers.{i}.attention.wo.weight')

    mapper.add_mapping('estimator.transformer.layers.{i}.feed_forward.w1.weight', 'transformer.layers.{i}.feed_forward.w1.weight')
    mapper.add_mapping('estimator.transformer.layers.{i}.feed_forward.w2.weight', 'transformer.layers.{i}.feed_forward.w2.weight')
    mapper.add_mapping('estimator.transformer.layers.{i}.feed_forward.w3.weight', 'transformer.layers.{i}.feed_forward.w3.weight')

    # RMSNorm weights (now using RMSNorm instead of LayerNorm with affine=False)
    mapper.add_mapping('estimator.transformer.layers.{i}.attention_norm.norm.weight', 'transformer.layers.{i}.attention_norm.weight')
    mapper.add_mapping('estimator.transformer.layers.{i}.attention_norm.project_layer.(weight|bias)', 'transformer.layers.{i}.attention_norm_modulation.$2')

    mapper.add_mapping('estimator.transformer.layers.{i}.ffn_norm.norm.weight', 'transformer.layers.{i}.ffn_norm.weight')
    mapper.add_mapping('estimator.transformer.layers.{i}.ffn_norm.project_layer.(weight|bias)', 'transformer.layers.{i}.ffn_norm_modulation.$2')

    mapper.add_mapping('estimator.transformer.layers.{i}.skip_in_linear.(weight|bias)', 'transformer.layers.{i}.skip_in_linear.$2')
    ...
    mapper.add_mapping('estimator.transformer.norm.project_layer.(weight|bias)', 'transformer.norm_modulation.$1')
    ...
    mapper.add_mapping('estimator.wavenet.cond_layer.conv.conv.(weight_v|weight_g|weight|bias)', 'wavenet.cond_layer.$1')

    # Dilated convolution layers (per-block)
    mapper.add_mapping('estimator.wavenet.in_layers.{i}.conv.conv.(weight_v|weight_g|weight|bias)', 'wavenet.in_layers.{i}.conv.$2')

    # Residual/skip projection layers (separate from blocks, matches PyTorch)
    mapper.add_mapping('estimator.wavenet.res_skip_layers.{i}.conv.conv.(weight_v|weight_g|weight|bias)', 'wavenet.res_skip_layers.{i}.$2')
    ...
    mapper.add_mapping('estimator.final_layer.linear.(weight_v|weight_g|weight|bias)', 'final_layer.linear.$1')
    mapper.add_mapping('estimator.final_layer.adaLN_modulation.1.(weight|bias)', 'final_layer.adaLN_modulation.layers.1.$1')
...
def load_dit_weights(
    ...
    for pt_name, pt_tensor in pytorch_checkpoint.items():
        ...
        # Transpose Conv1d weights: PyTorch (out, in, kernel) -> MLX (out, kernel, in)
        if mlx_value.ndim == 3 and '.weight' in mlx_path and ('conv' in mlx_path.lower() or 'cond_layer' in mlx_path or 'res_skip' in mlx_path):
            mlx_value = mx.transpose(mlx_value, (0, 2, 1))
