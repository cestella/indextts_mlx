...
def load_semantic_codec_model(model: RepCodec, weights_path: str) -> RepCodec:
    ...
    # Final layer norm
    model.encoder[0].final_layer_norm.weight = encoder_quantizer_weights[
        f"{prefix}.final_layer_norm.weight"
    ]
    model.encoder[0].final_layer_norm.bias = encoder_quantizer_weights[
    ...
    for q_idx in range(num_quantizers):
        ...
        if f"{q_prefix}.out_project.weight" in encoder_quantizer_weights:
            quantizer.out_project.weight = encoder_quantizer_weights[
                f"{q_prefix}.out_project.weight"
            ]
