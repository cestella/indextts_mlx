...
def _load_conformer_encoder(encoder, flat_weights: Dict, prefix: str):
    ...
    for layer_idx in range(len(encoder.encoders)):
        ...
        layer.conv_module.pointwise_conv1.weight = flat_weights[
            f"{lp}.conv_module.pointwise_conv1.weight"
        ]
        layer.conv_module.pointwise_conv1.bias = flat_weights[
            f"{lp}.conv_module.pointwise_conv1.bias"
        ]
        layer.conv_module.depthwise_conv.weight = flat_weights[
            f"{lp}.conv_module.depthwise_conv.weight"
        ]
        layer.conv_module.depthwise_conv.bias = flat_weights[
            f"{lp}.conv_module.depthwise_conv.bias"
        ]
        ...
        layer.conv_module.pointwise_conv2.weight = flat_weights[
            f"{lp}.conv_module.pointwise_conv2.weight"
        ]
        layer.conv_module.pointwise_conv2.bias = flat_weights[
            f"{lp}.conv_module.pointwise_conv2.bias"
        ]
...
def load_gpt_model(model: UnifiedVoice, weights_path: str) -> UnifiedVoice:
    ...
    _load_conformer_encoder(
        model.emo_conditioning_encoder, flat_weights, "emo_conditioning_encoder"
    )
